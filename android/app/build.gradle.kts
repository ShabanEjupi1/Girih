...
//   python linux-install/tools/firebase-konfigurimi.py <celesi.json> merr tech.spacecode.girih
if (file("google-services.json").exists()) {
    ...
    if (eshteLeshim) {
        throw GradleException(
            ...
            "i vdekur te ky leshim. Merre me firebase-konfigurimi.py (paketa tech.spacecode.girih) " +
...
//
// !! Çelësi `girih-upload.jks` është KOPJE E VETME te `spacecode-brain/keys/`.
//    Nëse humbet, ky aplikacion nuk përditësohet dot më kurrë: Android-i
//    refuzon një paketë të nënshkruar me çelës tjetër, dhe Play-i do të donte
//    paketë e listim krejt të ri.
...
android {
    namespace = "tech.spacecode.girih"
    ...
    defaultConfig {
        applicationId = "tech.spacecode.girih"
        minSdk = 24
        // 🚨 Play kërkon **API 36 për çdo ngarkim të ri që nga 31 gushti 2026**.
        // Me një numër më të ulët ngarkimi refuzohet te dera, para se ta shohë
        // njeri. Ky aplikacion nis drejt e me 36, ndaj s'ka çfarë ngrihet vitin
        // që vjen në panik.
        targetSdk = 36
        ...
        val mesazhi = """
            ⛔ Nënshkrimi do të binte prapa te çelësi i DEBUG-ut.
               ...
               kura            : krijo `android/key.properties` me shteg ABSOLUT —
                 storeFile=/mnt/data/workspace/spacecode-brain/keys/girih-upload.jks
                 storePassword=… · keyAlias=girih · keyPassword=…
